...
if __name__ == '__main__':
    ...
    callbacks = [
        ...
        lgb.log_evaluation(period=0)  # 0だとログ出力なし。10などにすればその間隔でログ表示
    ]

    # CV 実行
    history = lgb.cv(params=params,
                    train_set=gbm_train,
                    nfold=5,
...
#                     early_stopping_rounds=10, verbose_eval=False)
    ...
    # get result
    cv_auc = history['valid auc-mean'][-1]
    best_rounds = len(history['valid auc-mean'])
    ...
    with mlflow.start_run():
        ...
        mlflow.lightgbm.log_model(
            ...
            registered_model_name="adult_income_predictor"
        )
        
        print(f"\nModel saved to MLflow with run ID: {mlflow.active_run().info.run_id}")
        ...
        print(f"1. Local REST API server:")
        print(f"   mlflow models serve -m {model_uri} -p 5000")
        print(f"2. Register model for production:")
        print(f"   Use MLflow UI or programmatically register the model")
        print(f"3. Deploy to cloud platforms (AWS SageMaker, Azure ML, etc.)")
        print(f"   Use the model URI: {model_uri}")
